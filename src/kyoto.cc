...
static SourceLocation LexLoc = {1, 
0};
static int advance() {
...
public:
  ...
  raw_ostream &dump(raw_ostream &out, int ind) override {
    ExprAST::dump(
out << "binary" << Op, ind);
    LHS->dump(indent(out, ind) << "LHS:", 
ind + 1);
    RHS->dump(indent(out, ind) << "RHS:", 
ind + 1);
    return out;
  }
};
...
public:
  ...
  raw_ostream &dump(raw_ostream &out, int ind) override {
    ExprAST::dump(
out << "if", ind);
    Cond->dump(indent(out, ind) << "Cond:", 
ind + 1);
    Then->dump(indent(out, ind) << "Then:", 
ind + 1);
    Else->dump(indent(out, ind) << "Else:", 
ind + 1);
    return out;
  }
};
...
public:
  ...
  raw_ostream &dump(raw_ostream &out, int ind) override {
    ExprAST::dump(
out << "for", ind);
    Start->dump(indent(out, ind) << "Cond:", 
ind + 1);
    End->dump(indent(out, ind) << "End:", 
ind + 1);
    Step->dump(indent(out, ind) << "Step:", 
ind + 1);
    Body->dump(indent(out, ind) << "Body:", 
ind + 1);
    return out;
  }
};
...
static std::unique_ptr<ExprAST> ParseIdentifierExpr() {
  ...
  if (CurTok != '(') // Simple variable ref.
    return std::make_unique<VariableExprAST>(LitLoc, 
IdName);
  // Call.
  getNextToken(); // eat (
  std::vector<std::unique_ptr<ExprAST>> Args;
...
CurTok) || CurTok == '(' || CurTok == ',')
  ...
  if (auto Operand = ParseUnary())
    return std::make_unique<UnaryExprAST>(Opc, 
std::move(Operand));
  return nullptr;
}
/// binoprhs
...
static std::unique_ptr<ExprAST> ParseBinOpRHS(int ExprPrec,
  ...
  while (true) {
    ...
    if (TokPrec < NextPrec) {
      RHS = ParseBinOpRHS(TokPrec + 1, 
std::move(RHS));
      if (!RHS)
        return nullptr;
    }
    // Merge LHS/RHS.
    LHS = std::make_unique<BinaryExprAST>(BinLoc, BinOp, 
std::move(LHS),
...
this);
  // Load the value.
  return Builder->CreateLoad(Type::getDoubleTy(*TheContext), V, 
Name.c_str());
}
...
"Unknown unary operator");
  KSDbgInfo.emitLocation(
this);
  return Builder->CreateCall(F, OperandV, 
"unop");
}
...
MergeBB);
  ...
  // Emit else block.
  TheFunction->insert(TheFunction->end(), 
ElseBB);
...
LoopBB);
  ...
  if (Step) {
    ...
    StepVal = ConstantFP::get(*TheContext, 
APFloat(1.0));
  }
  ...
  Value *CurVar = Builder->CreateLoad(Type::getDoubleTy(*TheContext), 
Alloca,
...
"loopcond");
  ...
  BasicBlock *AfterBB =
      BasicBlock::Create(*TheContext, "afterloop", 
TheFunction);
  // Insert the conditional branch into the end of LoopEndBB.
  Builder->CreateCondBr(EndCond, LoopBB, 
AfterBB);
...
VarName);
  ...
  return Constant::getNullValue(Type::getDoubleTy(
*TheContext));
}
...
Function *PrototypeAST::codegen() {
  ...
  std::vector<Type *> Doubles(Args.size(), Type::getDoubleTy(
*TheContext));
  FunctionType *FT =
      FunctionType::get(Type::getDoubleTy(*TheContext), Doubles, 
false);
  Function *F =
...
Function *FunctionAST::codegen() {
  ...
  Function *TheFunction = getFunction(
P.getName());
  if (!TheFunction)
    return nullptr;
...
KSDbgInfo.TheCU->getDirectory());
  ...
  DISubprogram *SP = DBuilder->createFunction(
      FContext, P.getName(), StringRef(), 
Unit, LineNo,
      CreateFunctionType(
TheFunction->arg_size()), ScopeLine,
      DINode::FlagPrototyped, 
DISubprogram::SPFlagDefinition);
  TheFunction->setSubprogram(SP);
  // Push the current scope.
  KSDbgInfo.LexicalBlocks.push_back(
SP);
...
true);
    DBuilder->insertDeclare(Alloca, D, 
DBuilder->createExpression(),
                            DILocation::get(SP->getContext(), LineNo, 0, 
SP),
...
*TheFunction);
  ...
  if (P.isBinaryOp())
    BinopPrecedence.erase(
Proto->getOperatorName());
...
*TheContext);
  ...
  Builder = std::make_unique<IRBuilder<>>(
*TheContext);
}
...
int main() {
  ...
  TheModule->addModuleFlag(Module::Warning, 
"Debug Info Version",
...
2);
  ...
  DBuilder = std::make_unique<DIBuilder>(
*TheModule);
