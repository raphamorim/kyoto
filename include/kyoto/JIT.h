...
public:
  ...
  Error add(ResourceTrackerSP RT, std::unique_ptr<FunctionAST> F) {
    return RT->getJITDylib().define(
        std::make_unique<KaleidoscopeASTMaterializationUnit>(
*this,
...
KaleidoscopeASTMaterializationUnit::KaleidoscopeASTMaterializationUnit(
    ...
    : MaterializationUnit(L.getInterface(
*F)), L(L), F(std::move(F)) {}
...
std::move(*EPC));
    auto EPCIU = EPCIndirectionUtils::Create(
ES->getExecutorProcessControl());
    if (!EPCIU)
      return EPCIU.takeError();
    (*EPCIU)->createLazyCallThroughManager(
        *ES, ExecutorAddr::fromPtr(
&handleLazyCallThroughError));
